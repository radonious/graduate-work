...
class TokenAnalyzer {
     ...
     */
    fun tokensAnalyzeDetails(tokens1: List<String>, tokens2: List<String>, k: Int = 3): Map<String, Double> {
        return mapOf(
     ...
     */
    fun tokensAnalyzeFinalScore(tokens1: List<String>, tokens2: List<String>, k: Int = 3): Double {
        ...
        val baseScore =
            lcs * LCS_SCORE_WEIGHT + demerau * DEMERAU_SCORE_WEIGHT + jaccard * JACCARD_SCORE_WEIGHT + gram * K_GRAM_SCORE_WEIGHT
...
fun main22() {
    ...
    val tokens1 = tp.parseTokens(CodeSnippets.restructuredBSearch())
    val tokens2 = tp.parseTokens(CodeSnippets.renameBSearch())
    val tokens3 = tp.parseTokens(CodeSnippets.longCode())

    val result = ta.tokensAnalyzeDetails(tokens1, tokens2)
    val result2 = ta.tokensAnalyzeDetails(tokens1, tokens3)

    println("\ncode1 and code2")
    result.forEach { (metric, value) -> println("$metric: $value") }
    println(ta.tokensAnalyzeFinalScore(tokens1, tokens2))

    println("\ncode1 and code3")
    result2.forEach { (metric, value) -> println("$metric: $value") }
    println(ta.tokensAnalyzeFinalScore(tokens1, tokens3))
}
